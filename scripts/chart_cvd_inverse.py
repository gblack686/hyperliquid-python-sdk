...
from common import fetch_agg_trades, ts_range, DAPI, INVERSE_SYMBOL, OUT_DIR
...
tr = fetch_agg_trades(DAPI, INVERSE_SYMBOL, start_ms, end_ms)
if tr:
    df = pd.DataFrame(tr)
...
plt.title(f"CVD Inverse ({INVERSE_SYMBOL}) — last {args.lookback_min} min")
plt.xlabel("Time (UTC)"); plt.ylabel("CVD (BTC)")
os.makedirs(OUT_DIR, exist_ok=True)
out = os.path.join(OUT_DIR, "cvd_inverse.png")
