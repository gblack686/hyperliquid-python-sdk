---
name: hyp-orderbook
description: View orderbook (bids/asks) for a Hyperliquid ticker
argument-hint: "<ticker> [depth] - e.g., BTC 10"
---

## Fetch Hyperliquid Orderbook

Get the L2 orderbook for a specified ticker. Default depth is 5 levels.

Run this Python script with the user's specified ticker:
...
async def get_orderbook(ticker, depth=5):
    ...
    asks = list(reversed(book.get('asks', [])[:depth]))
...
Execute with: `python scripts/hyp_orderbook.py <TICKER> [DEPTH]`
